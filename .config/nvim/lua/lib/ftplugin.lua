...
M.extend = function(name, new_config)
    ...
    conf.abbr =
        vim.tbl_deep_extend("force", conf.abbr or {}, new_config.abbr or {})
    conf.opt =
        vim.tbl_deep_extend("force", conf.opt or {}, new_config.opt or {})
    conf.bufvar =
        vim.tbl_deep_extend("force", conf.bufvar or {}, new_config.bufvar or {})
    ...
    for k, v in pairs(conf.opt) do
        ...
        if opt_info.scope == "win" then
            ...
            if not ok then
                vim.notify(
                    string.format(
                        "Error setting window option %s = %s: %s",
                        k,
                        vim.inspect(v),
                        err
                    ),
    ...
    if conf.opt then
        for k, v in pairs(conf.opt) do
            ...
            if opt_info.scope == "buf" then
                local ok, err =
                    pcall(vim.api.nvim_set_option_value, k, v, { buf = bufnr })
                if not ok then
                    vim.notify(
                        string.format(
                            "Error setting buffer option %s = %s: %s",
                            k,
                            vim.inspect(v),
                            err
                        ),
...
M.setup = function(opts)
    ...
    vim.api.nvim_create_autocmd("BufWinEnter", {
        ...
        callback = function(params)
            ...
            -- If in a terminal buffer, make the filetype "terminal"
